...
const timelineItems = [
    {
        year: "1997年",
        events:[
            {month: "10月", content: "有限会社ミップを岸和田市中北町にて創立"}
        ]
    },
    {
        year: "1998年",
        events:[
            {month: "1月", content: "財団法人阪神・淡路産業復興推進機構と共に無災地域での人材育成機関としてデジタル映像工房を開設しクリエーター育成をおこなう"}
        ]
    },
    {
        year: "2001年",
        events:[
            {month: "4月", content: "サーバアウトソーシング事業 GNEUGUIA.NET（ナギアネット）をサービス提供開始"},
            {month: "6月", content: "経済産業省・大阪大学分子制御内科学（第2内科）・N2システム（株）共同プロジェクト実地医科向け糖尿病治療支援システムに参加"},
    ]
    },
    {
        year: "2002年",
        events:[
            {month: "5月", content: "経済産業省・大阪大学分子制御内科学（第2内科）・N2システム（株）共同プロジェクト治療概念スキーマによる知的病身連携システムの 研究開発に参加"},
            {month: "8月", content: "専門医向け特定疾患治療支援システム開発に従事"},
    ]
    },
    {
        year: "2005年",
        events:[
            {month: "12月", content: "全国営業拠点サポート販促資材運用支援システムをサービス提供開始"}
        ]
    },
    {
        year: "2006年",
        events:[
            {month: "4月", content: "販促資材運用支援システムが大手製薬メーカ様の全国の営業所と倉庫連携サービスがスタート"},
            {month: "8月", content: "CALib（カリブ）をサービス提供開始"},
    ]
    },
    {
        year: "2007年",
        events:[
            {month: "3月", content: "複数拠点間でのデータファイルストックヤードVisWorks（ビズワークス）をサービス開始"}
        ]
    },
    {
        year: "2008年",
        events:[
            {month: "11月", content: "ISO/IEC 27001:2005　/　JIS Q 27001:2006を認証取得"}
        ]
    },
    {
        year: "2014年",
        events:[
            {month: "5月",content: "vmware partnerとして認定を受ける(Partner with VMware Cloud Provider)ミップ　クラウドサービス開始"}
        ]
    },
    {
        year: "2015年",
        events:[
            {month: "5月",content: "株式会社わかさ生活と資本業務提携"}
        ]
    },
    ...
    return (
        <>
            ...
            <div className="max-w-[1000px] m-auto">
                <section className="mt-[50px] text-[#0354A5]">
            ...
            {/* ーーーー 強み ーーーー */}
            <section className="max-w-[1000px] m-auto mb-4 text-[#0354A5]">
                <p className="text-[20px] text-main mt-50">MIPの強み</p>
                <h2 className="text-[64px] font-bold text-main">Strengths</h2>
            </section>
            <div className="bg-gradient-to-tr from-[#f28900] to-[#ffcf4a]">
                <div className="text-[#fff] pb-[100px] pt-[100px] flex flex-col items-center">
            ...
            {/* ーーーー 取引先 ーーーー */}
            <div className="m-auto mb-4 text-[#0354A5]">
                <div className="max-w-[1000px] m-auto">
                    <p className="text-[20px] text-main mt-50">主な取引先</p>
                    <h2 className="text-[64px] font-bold text-main">Customer</h2>
                </div>
                <section className="py-[60px] bg-[#fff] shadow-[0_0_10px_rgba(0,0,0,0.2)]">
                    <div className="max-w-[1000px] m-auto">
            ...
            <div className="max-w-[1000px] m-auto">
                <section className="mb-4 text-[#0354A5]">
                    <div>
                        <p className="text-[20px] text-main mt-50">沿革</p>
                        <h2 className="text-[64px] font-bold text-main">History</h2>
                    </div>
                    
                    <section className="font-bold grid grid-cols-2">
                        {timelineItems.map((item,index)=>(
                            index % 2 === 0? (
                                <>
                                    <div key={`left-${index}`} className="flex items-center bg-[#fff] border-[#0354A5] border-1 px-[20px] py-[30px]">
                                        <h3 className="text-[20px] mr-[25px]">{item.year}</h3>
                                        <div>
                                    ...
                                    <div key={`right-blank-${index}`}></div>
                                </>
                            ):(
                                <>
                                    <div key={`left-blank-${index}`}></div>
                                    <div key={`right-${index}`} className="flex items-center bg-[#fff] border-[#0354A5] border-1 px-[20px] py-[30px]">
                                        <h3 className="text-[20px] mr-[25px]">{item.year}</h3>
                                        <div>
                                            {item.events.map((event,index)=>(
                                                <div key={index} className="flex text-[14px]">
            ...
            {/* ーーーー アクセス ーーーー */}
            <section className="text-[#0354A5] mb-[140px]">
                    ...
                    <section className="flex justify-center items-center gap-[40px]">
                        ...
                        <section className=" flex flex-col">
                            <address className="text-[20px] flex flex-col gap-[34px]">
                                <p>〒531-0072<br/>
                            ...
                            <div className="flex items-center justify-end text-[20px] mt-[40px] ">
                                <p>Googleマップ</p>
